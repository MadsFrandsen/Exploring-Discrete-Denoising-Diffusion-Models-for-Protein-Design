...
class UNet_v3(nn.Module):
    ...
    def forward(self, input, time):
        ...
        input = utils.normalize_data(input.float())
        ...
        for layer in self.up:
            if isinstance(layer, ResBlockWithAttention):
                input = layer(torch.cat((input, feats.pop()), 1), time_embed)
        ...
        if self.model_output == 'logistic_pars':
            ...
            loc = torch.tanh(loc + input_start), log_scale
            return loc, log_scale
        else:
            out = out.reshape(out.shape[0], out.shape[1], out.shape[2], self.out_channel, self.num_pixel_vals)
